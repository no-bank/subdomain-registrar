...
contract SubdomainStorage {
    ...
    function _setTwitterURI(bytes32 node, string calldata _tokenURI) external onlyController {
        uint256 tokenId = uint256(node);
    ...
    function doRegistration(uint256 tokenId, uint duration, string calldata _tokenURI) external onlyController returns(uint) {
    ...
    function renew(bytes32 node, uint duration) external onlyController returns(uint expire) {
        uint256 tokenId = uint256(node);
