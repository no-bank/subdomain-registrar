...
async function configureDomain(accounts) {
    ...
    var tx = await subdomainRegistrar.configureDomain("sefwallet", utils.toBN('10000000000000000'), accounts[0], {from: accounts[0]});
...
module.exports = async function(callback) {
    ...
    async function isNameAvailable(host, duration) {
        ...
        var ens = new ENS({ provider: web3.currentProvider, ensAddress: mainnet_ens_address });
        ...
        if (found == ethers.constants.AddressZero) {
            ...
            return {address: found, rentPrice: rentPrice, subdomainRegistrar: subdomainRegisterAddress};
        }
    ...
    try {
        ...
        var available = await isNameAvailable("superlongcheaptest2.sefwallet.one", 200000000);
        console.log(available);
        if(available.address == ethers.constants.AddressZero) {
            var ens = await Registry.at(mainnet_ens_address);
            var subdomainRegistrar = await SubdomainRegistrar.at(available.subdomainRegistrar)
            var publicresolver = await ens.resolver(namehash.hash("resolver.one"));
            console.log("Public Resolver", await ens.resolver(namehash.hash("crazy.one")))
            console.log("Resolver=", publicresolver);
            var tx = await subdomainRegistrar.register(SEF_LABEL, 'superlongcheaptest2', accounts[0], duration, '', publicresolver, {
                from: accounts[0],
