...
module.exports = async function(deployer, network, accounts) {
    ...
    await deployer.deploy(SubdomainRegistrar, ensAddress, priceOracle.address, subdomainStorage.address, {from: accounts[0]});
    ...
    console.log("Available on controller=", await firstController.available(name))

    var rentPrice = await firstController.rentPrice(name, duration);
    console.log("rentprice=", rentPrice.toString());

    var commitment = await firstController.makeCommitment(name, accounts[0], secret);
    ...
    var tx = await ens.setResolver(SEF_NODE, resolver_address, {from: accounts[0]});
